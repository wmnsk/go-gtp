...
func NewBearerTFT(op uint8, filters []*TFTPacketFilter, ids []uint8, params []*TFTParameter) *IE {
	v := NewBearerTFTFields(op, filters, ids, params)
	b, err := v.Marshal()
	if err != nil {
		return nil
	}
...
// BearerTFT returns BearerTFT in BearerTFTFields type if the type of IE matches.
func (i *IE) BearerTFT() (*BearerTFTFields, error) {
	switch i.Type {
	case BearerTFT:
		return ParseBearerTFTFields(i.Payload)
	case BearerContext:
		ies, err := i.BearerContext()
		if err != nil {
			return nil, fmt.Errorf("failed to retrieve BearerTFT: %w", err)
		}

		for _, child := range ies {
			if child.Type == BearerTFT {
				return child.BearerTFT()
			}
		}
...
// BearerTFTFields is a set of fields in BearerTFT IE.
type BearerTFTFields struct {
	OperationCode           uint8
...
// NewBearerTFTFields creates a new BearerTFTFields.
func NewBearerTFTFields(op uint8, filters []*TFTPacketFilter, ids []uint8, params []*TFTParameter) *BearerTFTFields {
...
// Marshal serializes BearerTFTFields.
func (f *BearerTFTFields) Marshal() ([]byte, error) {
...
// MarshalTo serializes BearerTFTFields.
func (f *BearerTFTFields) MarshalTo(b []byte) error {
	if len(b) < 1 {
...
// ParseBearerTFTFields decodes BearerTFTFields.
func ParseBearerTFTFields(b []byte) (*BearerTFTFields, error) {
	f := &BearerTFTFields{}
...
// UnmarshalBinary decodes given bytes into BearerTFTFields.
func (f *BearerTFTFields) UnmarshalBinary(b []byte) error {
	if len(b) < 1 {
		return io.ErrUnexpectedEOF
	}
...
// MarshalLen returns the serial length of BearerTFTFields in int.
func (f *BearerTFTFields) MarshalLen() int {
	l := 1
